...
while true; do 
	if [ "$CAMGRID_METHOD" == "desktop_xfce" ]; then
		ffmpeg -loglevel fatal -threads 1 -stimeout $RTSP_TIMEOUT -skip_frame nokey -i "$STREAM_URL" -vf fps=fps=$CAPTURE_FPS -update 1 -an -y -s $CAPTURE_RESOLUTION "$CAPTURE_LOCATION/$STREAM_TITLE.$CAPTURE_FORMAT" </dev/null; 
	elif [ "$CAMGRID_METHOD" == "direct_to_framebuffer" ]; then
		ffmpeg -loglevel fatal -threads 1 -stimeout $RTSP_TIMEOUT -skip_frame nokey -i "$STREAM_URL" -an -y -filter_complex "setpts=PTS-STARTPTS, fps=fps=$CAPTURE_FPS, scale=$CAPTURE_RESOLUTION" -pix_fmt bgra -update 1 -f fbdev -xoffset $XOFFSET -yoffset $YOFFSET /dev/fb0 </dev/null;
	fi
done
