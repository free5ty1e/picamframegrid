...
for i in "${!RTSP_STREAM_URLS[@]}"; do 
	...
	STREAM_RESOLUTION=${CAPTURE_RESOLUTIONS[$i]}
...
# 	FFMPEG_FILTER_PARAMETERS_B="$FFMPEG_FILTER_PARAMETERS_B[video$i] overlay=shortest=0:x=$XVAL"
	...
	camgridstartframecapturerpizero2w.sh "$STREAM_URL" "$STREAM_TITLE" "$STREAM_XOFFSET" "$STREAM_YOFFSET" "$STREAM_FPS" "$STREAM_RESOLUTION" &
