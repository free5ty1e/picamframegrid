...
while true; do 
	if [ "$CAMGRID_METHOD" == "desktop_xfce" ]; then
		...
		nice -10 ffmpeg -threads 1 -timeout $RTSP_TIMEOUT -flags2 fast -err_detect ignore_err -i "$STREAM_URL" -pix_fmt yuv420p -an -y -f fbdev -xoffset $XOFFSET -yoffset $YOFFSET -c copy -s $CAPTURE_RESOLUTION /dev/fb0 </dev/null;
