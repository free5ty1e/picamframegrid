...
while true; do 
	if [ "$CAMGRID_METHOD" == "desktop_xfce" ]; then
		...
		nice -10 ffmpeg -threads 1 -timeout $RTSP_TIMEOUT -lowres 0 -skip_loop_filter 1 -skip_frame nokey -flags2 fast -ec favor_inter -idct simpleauto -err_detect ignore_err -strict unofficial -bug autodetect -i "$STREAM_URL" -an -y -pix_fmt rgb565le -update 1 -f fbdev -xoffset $XOFFSET -yoffset $YOFFSET -c copy -s $CAPTURE_RESOLUTION /dev/fb0 </dev/null;
