...
while true; do 
	if [ "$CAMGRID_METHOD" == "desktop_xfce" ]; then
		...
		nice -10 ffmpeg -threads 1 -timeout $RTSP_TIMEOUT -err_detect ignore_err -fflags nobuffer -flags low_delay \
			-i "$STREAM_URL" -vf 'setpts=PTS-STARTPTS' \
			-pix_fmt yuv420p -r 10 -c:v h264_v4l2m2m -crf 30 \
			-preset ultrafast \
			-xoffset $XOFFSET -yoffset $YOFFSET -s $CAPTURE_RESOLUTION  \
			-an -y -f fbdev /dev/fb0 </dev/null;
		# -loglevel fatal
