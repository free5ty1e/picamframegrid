...
while true; do 
	if [ "$CAMGRID_METHOD" == "desktop_xfce" ]; then
		...
		nice -10 ffmpeg -threads 1 -timeout $RTSP_TIMEOUT -preset ultrafast -c:a copy -err_detect ignore_err -i "$STREAM_URL" -pix_fmt rgb565le -an -y -f fbdev -xoffset $XOFFSET -yoffset $YOFFSET -s $CAPTURE_RESOLUTION /dev/fb0 </dev/null;
