...
while true; do 
	if [ "$CAMGRID_METHOD" == "desktop_xfce" ]; then
		...
		nice -10 ffmpeg -threads 1 -timeout $RTSP_TIMEOUT -flags2 fast -err_detect ignore_err -pix_fmt rgb565le -i "$STREAM_URL" -an -y -f fbdev -xoffset $XOFFSET -yoffset $YOFFSET -c copy -s $CAPTURE_RESOLUTION /dev/fb0 </dev/null;
