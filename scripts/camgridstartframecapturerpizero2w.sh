...
while true; do 
	if [ "$CAMGRID_METHOD" == "desktop_xfce" ]; then
		...
		# -vf "select=gt(n,prev_n)" is supposed to discard all but the most recent frame, basically skip any frames we weren't able to render in time.
		nice -10 ffmpeg -loglevel fatal -threads 1 -timeout $RTSP_TIMEOUT -err_detect ignore_err -i "$STREAM_URL" -pix_fmt rgb565le -preset ultrafast -vf "select=gt(n,prev_n)" -c:a copy -an -y -f fbdev -xoffset $XOFFSET -yoffset $YOFFSET -s $CAPTURE_RESOLUTION /dev/fb0 </dev/null;
